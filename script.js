...
const isExactChangePossible = (cid_sum, cid_rev, change) => {
      ...
      for(let i=0; i<cid_rev.length; i++){
        while(currency_values[cid_rev[i][0]] <= change && cidObj[cid_rev[i][0]]>0) {
          console.log(`Change is: ${change}`);
          console.log(`GOT: ${currency_values[cid_rev[i][0]]} for ${cid_rev[i][0]}`)
          const numberOfCurrency = Math.floor(change/currency_values[cid_rev[i][0]]);
          ...
          if(cidObj[cid_rev[i][0]] >= numberOfCurrency) {
            ...
            result += `${cid_rev[i][0]}: $${numberOfCurrency*currency_values[cid_rev[i][0]]} `
            displayAndUpdateCID(cid_rev.slice().reverse());
            if(change == 0)
              return true;
          }
            ...
            change = Number(change.toFixed(2))
            result += `${cid_rev[i][0]}: $${cidObj[cid_rev[i][0]]*currency_values[cid_rev[i][0]]} `
            displayAndUpdateCID(cid_rev.slice().reverse());
            if(change === 0)
              return true;
          }
